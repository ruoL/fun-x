...
(function () {
    ...
    // 定义网站根URL
    var _APPURL_ = $CONFIG.baseurl;
    
    var tmp = window.location.pathname,
        URL = _APPURL_ + 'static/editors/';//这里你可以配置成ueditor目录在您网站的相对路径或者绝对路径（指以http开头的绝对路径）
        
    UEDITOR_CONFIG = {
        imagePath: _APPURL_, //图片文件夹所在的路径，用于显示时修正后台返回的图片url！具体图片保存路径需要在后台设置。！important
        ...
        'body{margin:8px;font-family:"Microsoft YaHei", Verdana, Arial, Tahoma;font-size:14px}' +
        //针对li的处理
        'li{clear:both}' +
        //设置段落间距
        ...
        snapscreenImgIsUseImagePath: 0, //是否使用上面定义的imagepath，如果为否，那么server端需要直接返回图片的完整路径
        messages:{
            ...
            snapScreenNotIETip: '截图功能需要在 IE 浏览器下使用。',
            snapScreenMsg:'截图上传失败，请检查你的 PHP 环境。'
        },
